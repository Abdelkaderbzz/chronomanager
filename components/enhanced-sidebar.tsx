...
interface EnhancedSidebarProps {
  ...
  onFavoriteList: (folderId: string, listId: string, isFavorite: boolean) => void
...
export default function EnhancedSidebar({
  ...
  const moveLists = (folderId: string, dragIndex: number, hoverIndex: number) => {
    const folder = folders.find((f) => f.id === folderId)
    if (!folder) return
  ...
  const filteredFolders = folders.filter((folder) => {
    ...
    const folderMatch = folder.name.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const favoriteLists = folders.flatMap((folder) =>
    folder.lists.filter((list) => list.isFavorite).map((list) => ({ ...list, folderId: folder.id })),
  )
  ...
  return (
    <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
      ...
      >
            ...
            <div className="mb-4">
              ...
              {favoriteLists.map((list) => (
                ...
                >
                  ...
                  {list.tags.length > 0 && <span className="ml-2 text-xs text-muted-foreground">{list.tags[0]}</span>}
                </Button>
              ))}
            ...
            <div className="space-y-1">
              {filteredFolders.map((folder, index) => (
                <FolderItem
                  ...
                  onFavoriteFolder={(isFavorite) => onFavoriteFolder(folder.id, isFavorite)}
                  onFavoriteList={(listId, isFavorite) => onFavoriteList(folder.id, listId, isFavorite)}
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditingFolder((prev) => (prev ? { ...prev, icon: e.target.value } : null))}
                />
              </div>
              <div className="grid gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    className="w-12 h-9 p-1"
                  />
                  <div className="w-8 h-8 rounded-full" style={{ backgroundColor: editingFolder?.color || "" }} />
                </div>
              </div>
            </div>
            <DialogFooter>
        ...
        {/* Add List Dialog */}
        <Dialog open={!!addingListToFolder} onOpenChange={(open) => !open && setAddingListToFolder(null)}>
          <DialogContent>
            <DialogHeader>
  ...
  return (
    <div className={`space-y-1 ${isDragging ? "opacity-50" : ""}`}>
      <div ref={dragDropRef} className="flex items-center">
        <Button variant="ghost" size="sm" className="w-6 h-6 p-0 mr-1" onClick={onToggleExpand}>
          {isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
        </Button>
        <Button
          variant={isActive ? "secondary" : "ghost"}
          className={cn("justify-start w-full h-8 px-2 font-normal", isActive && "font-medium")}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => onFavoriteFolder(!folder.isFavorite)}>
              {folder.isFavorite ? (
                <>
            ...
            <DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={onDeleteFolder}>
          ...
          >
            {folder.lists.map((list, listIndex) => (
              <ListItem
                ...
                onFavoriteList={(isFavorite) => onFavoriteList(list.id, isFavorite)}
                folderId={folder.id}
                moveLists={moveLists}
              />
            ))}
...
function ListItem({ list, index, isActive, onSelectList, onFavoriteList, folderId, moveLists }: ListItemProps) {
  const [{ isDragging }, drag] = useDrag({
    type: `LIST-${folderId}`,
    item: { index },
    collect: (monitor) => ({
  ...
  return (
    <div ref={dragDropRef} className={`flex items-center ${isDragging ? "opacity-50" : ""}`}>
      <Button
        variant={isActive ? "secondary" : "ghost"}
        className={cn("justify-start w-full h-8 px-2 font-normal", isActive && "font-medium")}
      ...
      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onFavoriteList(!list.isFavorite)}>
        {list.isFavorite ? (
