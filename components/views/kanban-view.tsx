...
export default function KanbanView({
  ...
  return (
    <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
      ...
      {/* Add Task Dialog */}
      <Dialog open={!!addingToStatus} onOpenChange={(open) => !open && setAddingToStatus(null)}>
        <DialogContent>
          <DialogHeader>
...
function TaskCard({ task, onEdit, onDelete }: TaskCardProps) {
  ...
  return (
    <div
      ref={drag}
      className={`bg-background rounded-md border p-3 shadow-sm cursor-grab ${isDragging ? "opacity-50" : ""}`}
      style={{
    ...
    >
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="text-xs font-medium" style={{ color: getPriorityColor(task.priority) }}>
