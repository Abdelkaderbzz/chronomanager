...
export default function FolderListSystem() {
  ...
  useEffect(() => {
    ...
    if (savedFolders) {
      ...
      if (savedActiveFolder && foldersWithFavorites.some((folder: Folder) => folder.id === savedActiveFolder)) {
        setActiveFolder(savedActiveFolder)

        if (savedActiveList) {
          const activeFolder = foldersWithFavorites.find((folder: Folder) => folder.id === savedActiveFolder)
          if (activeFolder && activeFolder.lists.some((list: List) => list.id === savedActiveList)) {
  ...
  const handleCreateList = (folderId: string, newList: Omit<List, "id" | "tasks">) => {
    const list: List = {
      id: uuidv4(),
      ...newList,
      tasks: [],
  ...
  const handleUpdateList = (folderId: string, updatedList: List) => {
    const updatedFolders = folders.map((folder) => {
      if (folder.id === folderId) {
        return {
          ...folder,
          lists: folder.lists.map((list) => (list.id === updatedList.id ? updatedList : list)),
        }
      }
      return folder
    })
  ...
  const handleDeleteList = (folderId: string, listId: string) => {
    ...
    if (activeList === listId) {
      const activeFolder = updatedFolders.find((folder) => folder.id === folderId)
  ...
  const handleUpdateTask = (folderId: string, listId: string, updatedTask: Task) => {
    ...
    if (folder) {
      ...
      if (list) {
        ...
        if (task) {
          ...
          const doneStatus = list.statuses.find((s) => s.id === "done" || s.name.toLowerCase() === "done")

          // If task is being moved to "done" status, trigger confetti
          if (doneStatus && task.status !== doneStatus.id && updatedTask.status === doneStatus.id) {
  ...
  const handleDeleteTask = (folderId: string, listId: string, taskId: string) => {
    const updatedFolders = folders.map((folder) => {
      if (folder.id === folderId) {
        return {
          ...folder,
  ...
  const handleChangeListView = (folderId: string, listId: string, viewType: ViewType) => {
    const updatedFolders = folders.map((folder) => {
      if (folder.id === folderId) {
        return {
          ...folder,
  ...
  const handleCreateStatus = (folderId: string, listId: string, newStatus: Omit<Status, "id" | "order">) => {
    const updatedFolders = folders.map((folder) => {
      if (folder.id === folderId) {
        return {
          ...folder,
          lists: folder.lists.map((list) => {
            if (list.id === listId) {
              const maxOrder = Math.max(...list.statuses.map((s) => s.order), -1)
              const status: Status = {
  ...
  const handleUpdateStatus = (folderId: string, listId: string, updatedStatus: Status) => {
    const updatedFolders = folders.map((folder) => {
      if (folder.id === folderId) {
        return {
          ...folder,
          lists: folder.lists.map((list) => {
            if (list.id === listId) {
              return {
                ...list,
                statuses: list.statuses.map((status) => (status.id === updatedStatus.id ? updatedStatus : status)),
              }
            }
            return list
          }),
  ...
  const handleDeleteStatus = (folderId: string, listId: string, statusId: string) => {
    // Find the list
    const folder = folders.find((f) => f.id === folderId)
    if (!folder) return
    ...
    const tasksWithStatus = list.tasks.filter((task) => task.status === statusId)

    if (tasksWithStatus.length > 0) {
      toast({
        title: "Cannot delete status",
        description: "There are tasks using this status. Move or delete them first.",
        variant: "destructive",
      })
      return
    }
  ...
  const handleReorderStatuses = (folderId: string, listId: string, reorderedStatuses: Status[]) => {
  ...
  const handleFavoriteFolder = (folderId: string, isFavorite: boolean) => {
    ...
    toast({
      ...
      description: isFavorite ? "Folder added to favorites" : "Folder removed from favorites",
    })
  }

  const handleFavoriteList = (folderId: string, listId: string, isFavorite: boolean) => {
    const updatedFolders = folders.map((folder) => {
      if (folder.id === folderId) {
        return {
          ...folder,
    ...
    toast({
      title: isFavorite ? "Added to favorites" : "Removed from favorites",
      description: isFavorite ? "List added to favorites" : "List removed from favorites",
    })
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <AnimatedBackground />
      <ConfettiCelebration show={showConfetti} duration={3000} onComplete={() => setShowConfetti(false)} />
